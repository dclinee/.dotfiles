...
| zinit 插件 | github.com | ghproxy.net → gh-proxy.com |
| vim-plug | github.com | ghproxy.net → gh-proxy.com |
| Starship 安装脚本 | starship.rs | ghproxy.net → gh-proxy.com |
| uv 安装脚本 | astral.sh | ghproxy.net → gh-proxy.com |
| rustup 安装脚本 | sh.rustup.rs | ghproxy.net → gh-proxy.com |
...
所有外部安装脚本（Homebrew、Starship、uv、rustup）均先下载到临时文件再执行（不使用 `curl | sh` 管道），避免中间人攻击注入。下载失败时自动回退到镜像源重试。
